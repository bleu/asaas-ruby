...
module Asaas
  class AsaasObject
    ...
    def self.construct_from(values, opts = {})
      values = Asaas::Util.symbolize_names(values)

      # work around protected #initialize_from for now
      new(values[:id]).send(:initialize_from, values, opts)
    end

    # Determines the equality of two Asaas objects. Asaas objects are
    # considered to be equal if they have the same set of values and each one
    # of those values is the same.
    def ==(other)
      other.is_a?(AsaasObject) &&
        @values == other.instance_variable_get(:@values)
    end

    # Hash equality. As with `#==`, we consider two equivalent Asaas objects
    # equal.
    def eql?(other)
    ...
    # As with equality in `#==` and `#eql?`, we hash two Asaas objects to the
    ...
    # * +opts+ - Options for +AsaasObject+ like an API key that will be reused
    #   on subsequent API calls.
    #
    # ==== Options
    #
    # * +:dirty+ - Whether values should be initiated as "dirty" (unsaved) and
    #   which applies only to new AsaasObjects being initiated under this
    #   AsaasObject. Defaults to true.
    def update_attributes(values, opts = {}, dirty: true)
      values.each do |k, v|
        add_accessors([k], values) unless metaclass.method_defined?(k.to_sym)
        @values[k] = Util.convert_to_asaas_object(v, opts)
    ...
    # Sets all keys within the AsaasObject as unsaved so that they will be
    # included with an update when #serialize_params is called. This method is
    # also recursive, so any AsaasObjects contained as values or which are
    ...
    # serialized.
    def marshal_dump
      # The AsaasClient instance in @opts is not serializable and is not
      # really a property of the AsaasObject, so we exclude it when
      # dumping
      opts = @opts.clone
      opts.delete(:client)
      [@values, opts]
    ...
    def serialize_params(options = {})
      ...
      @values.each do |k, v|
        ...
        #   3. Its value is a AsaasObject. A AsaasObject may contain modified
        #      values within in that its parent AsaasObject doesn't know about.
        #
        unsaved = @unsaved_values.include?(k)
        next unless options[:force] || unsaved || v.is_a?(AsaasObject)
    ...
    # `AsaasObject`, and metadata can have a wide range of different keys
        ...
        keys.each do |k|
          ...
          [k, :"#{k}=", :"#{k}?"].each do |method_name|
            ...
            begin
              ...
              #
              #     https://github.com/asaas/asaas-ruby/issues/749
              #
      ...
      begin
        ...
        raise NoMethodError,
              e.message + ".  HINT: The '#{name}' attribute was set in the " \
                          ...
                          "with the result returned by Asaas's API, probably as a " \
    ...
    # * +:opts:+ Options for AsaasObject like an API key.
    ...
    protected def serialize_params_value(value, original, unsaved, force, # rubocop:todo Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
      ...
      # making sure any time one is set, we convert it to a AsaasObject. This
      ...
      # existing array being held by a AsaasObject. This could happen for
      # example by appending a new hash onto `additional_owners` for an
      # account.
      elsif value.is_a?(Hash)
        Util.convert_to_asaas_object(value, @opts).serialize_params

      elsif value.is_a?(AsaasObject)
    ...
    private def empty_values(obj)
      values = case obj
               when Hash then obj
               when AsaasObject then obj.instance_variable_get(:@values)
               else
                 raise ArgumentError,
