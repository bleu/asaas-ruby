...
require "asaas/instrumentation"

module Asaas
  # AsaasClient executes requests against the Asaas API and allows a user to
  ...
  class AsaasClient
    ...
    def initialize(config_arg = {})
      ...
      @config = case config_arg
                when Hash
                  Asaas.config.reverse_duplicate_merge(config_arg)
                when Asaas::ConnectionManager
                  # Supports accepting a connection manager object for backwards
                  # compatibility only, and that use is DEPRECATED.
                  Asaas.config.dup
                when Asaas::AsaasConfiguration
                  config_arg
                when String
                  Asaas.config.reverse_duplicate_merge(
                    { api_key: config_arg }
                  )
                else
    ...
    # Gets a currently active `AsaasClient`. Set for the current thread when
    # `AsaasClient#request` is being run so that API operations being executed
    ...
    def self.default_client
      current_thread_context.default_client ||= AsaasClient.new(Asaas.config)
    end

    # A default connection manager for the current thread scoped to the
    # configuration object that may be provided.
    def self.default_connection_manager(config = Asaas.config)
    ...
    def self.should_retry?(error,
                           method:, num_retries:, config: Asaas.config)
      ...
      when EOFError, Errno::ECONNREFUSED, Errno::ECONNRESET, # rubocop:todo Lint/DuplicateBranch
        ...
        true
      when Asaas::AsaasError
        # The API may ask us not to retry (e.g. if doing so would be a no-op),
        # or advise us to retry (e.g. in cases of lock timeouts). Defer to
        # those instructions if given.
        return false if error.http_headers["asaas-should-retry"] == "false"
        return true if error.http_headers["asaas-should-retry"] == "true"
    ...
    def self.sleep_time(num_retries, config: Asaas.config)
    ...
    # Gets the connection manager in use for the current `AsaasClient`.
    #
    ...
    #
    def request
      old_asaas_client = self.class.current_thread_context.active_client
      self.class.current_thread_context.active_client = self

      if self.class.current_thread_context.last_responses&.key?(object_id)
        raise "calls to AsaasClient#request cannot be nested within a thread"
      end
      ...
      begin
        ...
        self.class.current_thread_context.active_client = old_asaas_client
    ...
    def execute_request(method, path,
      ...
      begin
        resp = AsaasResponse.from_net_http(http_resp)
      rescue JSON::ParserError
        raise general_api_error(http_resp.code.to_i, http_resp.body)
      end

      # If being called from `AsaasClient#request`, put the last response in
    ...
    # it to a AsaasObject. This should be used for any request where we expect
    ...
    #
    # If a block is passed, a AsaasHeadersOnlyResponse is returned as the
    # block is expected to do all the necessary body processing. If no block is
    # passed, then a AsaasStreamResponse is returned containing an IO stream
    ...
    def execute_request_stream(method, path,
      ...
      resp = AsaasHeadersOnlyResponse.from_net_http(http_resp)
    ...
    ERROR_MESSAGE_CONNECTION =
      ...
      "Asaas (%s). You may be seeing this message because your DNS is not " \
      "working or you don't have an internet connection.  To check, try " \
      "running `host asaas.com` from the command line."
    ERROR_MESSAGE_SSL =
      "Could not establish a secure connection to Asaas (%s), you " \
      "may need to upgrade your OpenSSL version. To check, try running " \
      "`openssl s_client -connect api.asaas.com:443` from the command " \
      "line."

    # Common error suffix sared by both connect and read timeout messages.
    ERROR_MESSAGE_TIMEOUT_SUFFIX =
      "Please check your internet connection and try again. " \
      "If this problem persists, you should check Asaas's service " \
      "status at https://status.asaas.com, or let us know at " \
      "support@asaas.com."

    ERROR_MESSAGE_TIMEOUT_CONNECT = (
      "Timed out connecting to Asaas (%s). " +
      ERROR_MESSAGE_TIMEOUT_SUFFIX
    ).freeze

    ERROR_MESSAGE_TIMEOUT_READ = (
      "Timed out communicating with Asaas (%s). " +
      ERROR_MESSAGE_TIMEOUT_SUFFIX
    ).freeze
    ...
    # A record representing any data that `AsaasClient` puts into
    ...
    # with future non-major changes.
    class ThreadContext
      # A `AsaasClient` that's been flagged as currently active within a
      # thread by `AsaasClient#request`. A client stays active until the
      # completion of the request block.
      attr_accessor :active_client

      # A default `AsaasClient` object for the thread. Used in all cases where
      # the user hasn't specified their own.
      attr_accessor :default_client

      # A temporary map of object IDs to responses from last executed API
      # calls. Used to return a responses from calls to `AsaasClient#request`.
      #
      # Stored in the thread data to make the use of a single `AsaasClient`
      # object safe across multiple threads. Stored as a map so that multiple
      # `AsaasClient` objects can run concurrently on the same thread.
      #
      # Responses are only left in as long as they're needed, which means
      # they're removed as soon as a call leaves `AsaasClient#request`, and
      ...
      # all `AsaasClient` objects on a particular thread, and created so as to
    ...
    # Access data stored for `AsaasClient` within the thread's current
    # context. Returns `ThreadContext`.
    #
    # For internal use only. Does not provide a stable API and may be broken
    # with future non-major changes.
    def self.current_thread_context
      Thread.current[:asaas_client__internal_use_only] ||= ThreadContext.new
    end
    ...
    private def execute_request_internal(method, path,
      ...
      context.account         = headers["Asaas-Account"]
      context.api_key         = api_key
      context.api_version     = headers["Asaas-Version"]
    ...
    private def check_api_key!(api_key)
      unless api_key
        raise AuthenticationError, "No API key provided. " \
                                   'Set your API key using "Asaas.api_key = <API-KEY>". ' \
                                   "You can generate API keys from the Asaas web interface. " \
                                   "See https://asaas.com/api for details, or email " \
                                   "support@asaas.com if you have any questions."
      end
      ...
      raise AuthenticationError, "Your API key is invalid, as it contains " \
                                 ...
                                 "Asaas web interface. See https://asaas.com/api for details, or " \
                                 "email support@asaas.com if you have any questions.)"
    end
      ...
      begin
        ...
        if config.enable_telemetry? && context.request_id
          ...
          @last_request_metrics =
            AsaasRequestMetrics.new(context.request_id, request_duration_ms, usage: usage)
        end
    ...
    private def notify_request_begin(context)
      ...
      Asaas::Instrumentation.notify(:request_begin, event)
    ...
    private def notify_request_end(context, duration, http_status, num_retries,
      ...
      request_context = Asaas::Instrumentation::RequestContext.new(
        duration: duration,
        context: context,
        header: headers
      )
      response_context = Asaas::Instrumentation::ResponseContext.new(
      ...
      Asaas::Instrumentation.notify(:request_end, event)

      # The name before `request_begin` was also added. Provided for backwards
      # compatibility.
      Asaas::Instrumentation.notify(:request, event)
    end
    ...
    private def handle_error_response(http_resp, context)
      begin
        ...
        raise AsaasError, "Indeterminate error" unless error_data
      rescue JSON::ParserError, AsaasError
        raise general_api_error(http_resp.code.to_i, http_resp.body)
      end
    ...
    private def specific_api_error(resp, error_data, context)
      Util.log_error("Asaas API error",
    ...
    private def handle_network_error(error, context, num_retries,
                                     api_base = nil)
      Util.log_error("Asaas network error",
      ...
      if errors.nil?
        message = "Unexpected error #{error.class.name} communicating " \
                  "with Asaas. Please let us know at support@asaas.com."
      end
    ...
    private def request_headers(api_key, method)
      user_agent = "Asaas/v1 RubyBindings/#{Asaas::VERSION}"
      user_agent += " " + format_app_info(Asaas.app_info) unless Asaas.app_info.nil?
      ...
      if config.enable_telemetry? && !@last_request_metrics.nil?
        headers["X-Asaas-Client-Telemetry"] = JSON.generate(
      ...
      headers["Asaas-Version"] = config.api_version if config.api_version
      headers["Asaas-Account"] = config.asaas_account if config.asaas_account

      user_agent = @system_profiler.user_agent
      begin
        headers.update(
          "X-Asaas-Client-User-Agent" => JSON.generate(user_agent)
        )
      rescue StandardError => e
        headers.update(
          "X-Asaas-Client-Raw-User-Agent" => user_agent.inspect,
          :error => "#{e} (#{e.class})"
        )
      end
    ...
    private def log_request(context, num_retries)
      Util.log_info("Request to Asaas API",
    ...
    private def log_response(context, request_start, status, body, resp)
      Util.log_info("Response from Asaas API",
                    account: context.account,
                    api_version: context.api_version,
    ...
    class RequestLogContext
      ...
      # a `Asaas-Version` header beyond what configuration information that we
    ...
    class SystemProfiler
      ...
      def user_agent
        ...
        {
          application: Asaas.app_info,
          bindings_version: Asaas::VERSION,
          lang: "ruby",
    ...
    # AsaasRequestMetrics tracks metadata to be reported to asaas for metrics
    # collection
    class AsaasRequestMetrics
      # The Asaas request ID of the response.
