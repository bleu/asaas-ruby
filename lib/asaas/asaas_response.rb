...
module Asaas
  ...
  module AsaasResponseBase
    ...
    def self.populate_for_net_http(resp, http_resp)
      resp.http_headers = AsaasResponseHeaders.from_net_http(http_resp)
      resp.http_status = http_resp.code.to_i
      resp.request_id = http_resp["request-id"]
    end
  end

  # AsaasResponse encapsulates some vitals of a response that came back from
  # the Asaas API.
  class AsaasResponse
    include AsaasResponseBase
    ...
    # Initializes a AsaasResponse object from a Net::HTTP::HTTPResponse
    # object.
    def self.from_net_http(http_resp)
      resp = AsaasResponse.new
      resp.data = JSON.parse(http_resp.body, symbolize_names: true)
      resp.http_body = http_resp.body
      AsaasResponseBase.populate_for_net_http(resp, http_resp)
      resp
    end
  end

  # We have to alias AsaasResponseHeaders to AsaasResponse::Headers, as this
  # class used to be embedded within AsaasResponse and we want to be backwards
  # compatible.
  AsaasResponse::Headers = AsaasResponseHeaders

  # AsaasHeadersOnlyResponse includes only header-related vitals of the
  ...
  # response in memory.
  class AsaasHeadersOnlyResponse
    include AsaasResponseBase

    # Initializes a AsaasHeadersOnlyResponse object from a
    # Net::HTTP::HTTPResponse object.
    def self.from_net_http(http_resp)
      resp = AsaasHeadersOnlyResponse.new
      AsaasResponseBase.populate_for_net_http(resp, http_resp)
      resp
    end
  end
