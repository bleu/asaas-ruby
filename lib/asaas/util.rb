...
module Asaas
  module Util
    ...
    # Options that should be copyable from one AsaasObject to another
    ...
    def self.object_classes
      @object_classes ||= Asaas::ObjectTypes.object_names_to_classes
    end
    ...
    def self.custom_method(resource, target, name, http_verb, http_path)
      ...
      target.define_singleton_method(name) do |id, params = {}, opts = {}|
        ...
        Util.convert_to_asaas_object_with_params(resp.data, params, opts)
      end
    end

    # Converts a hash of fields or an array of hashes into a +AsaasObject+ or
    # array of +AsaasObject+s. These new objects will be created as a concrete
    # type as dictated by their `object` field (e.g. an `object` value of
    # `charge` would create an instance of +Charge+), but if `object` is not
    # present or of an unknown type, the newly created instance will fall back
    # to being a +AsaasObject+.
    #
    # ==== Attributes
    #
    # * +data+ - Hash of fields and values to be converted into a AsaasObject.
    # * +params+ - Params for +AsaasObject+ like filters used in search that
    #   will be reused on subsequent API calls.
    # * +opts+ - Options for +AsaasObject+ like an API key that will be reused
    #   on subsequent API calls.
    def self.convert_to_asaas_object(data, opts = {})
      convert_to_asaas_object_with_params(data, {}, opts)
    end
    ...
    # * +opts+ - Options for +AsaasObject+ like an API key that will be reused
    #   on subsequent API calls.
    def self.convert_to_asaas_object_with_params(data, params, opts = {})
      opts = normalize_opts(opts)

      case data
      when Array
        data.map { |i| convert_to_asaas_object(i, opts) }
      when Hash
        # Try converting to a known object class.  If none available, fall back
        # to generic AsaasObject
        object_name = data[:object] || data["object"]
        obj = object_classes.fetch(object_name, AsaasObject)
    ...
    def self.log_error(message, data = {})
      config = data.delete(:config) || Asaas.config
      logger = config.logger || Asaas.logger
      if !logger.nil? ||
         (!config.log_level.nil? && config.log_level <= Asaas::LEVEL_ERROR)
        log_internal(message, data, color: :cyan, level: Asaas::LEVEL_ERROR,
                                    logger: Asaas.logger, out: $stderr)
      end
    end
    ...
    def self.log_info(message, data = {})
      ...
      logger = config.logger || Asaas.logger
      if !logger.nil? ||
         (!config.log_level.nil? && config.log_level <= Asaas::LEVEL_INFO)
        log_internal(message, data, color: :cyan, level: Asaas::LEVEL_INFO,
                                    logger: Asaas.logger, out: $stdout)
      end
    end
    ...
    def self.log_debug(message, data = {})
      ...
      logger = config.logger || Asaas.logger
      if !logger.nil? ||
         (!config.log_level.nil? && config.log_level <= Asaas::LEVEL_DEBUG)
        log_internal(message, data, color: :blue, level: Asaas::LEVEL_DEBUG,
                                    logger: Asaas.logger, out: $stdout)
      end
    end
    ...
    def self.request_id_dashboard_url(request_id, api_key)
      ...
      "https://dashboard.asaas.com/#{env}/logs/#{request_id}"
    end
