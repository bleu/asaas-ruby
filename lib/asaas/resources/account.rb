...
module Asaas
  # This is an object representing a Asaas account. You can retrieve it to see
  # properties on the account like its current requirements or if the account is
  # enabled to make live charges or receive payouts.
  #
  # For Custom accounts, the properties below are always returned. For other accounts, some properties are returned until that
  # account has started to go through Connect Onboarding. Once you create an [Account Link](https://asaas.com/docs/api/account_links)
  # for a Standard or Express account, some parameters are no longer returned. These are marked as **Custom Only** or **Custom and Express**
  # below. Learn about the differences [between accounts](https://asaas.com/docs/connect/accounts).
  class Account < APIResource
    ...
    def deauthorize(_client_id = nil, opts = {})
    ...
    private def serialize_additional_owners(legal_entity, additional_owners)
      ...
      additional_owners.each_with_index do |v, i|
        # We will almost always see a AsaasObject except in the case of a Hash
        ...
        # AsaasObjects that can detect appends and replace a hash with a
        # AsaasObject.
        update = v.is_a?(AsaasObject) ? v.serialize_params : v
